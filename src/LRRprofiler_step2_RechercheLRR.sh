...
do
    ...
    if [[ $(wc -l ${outfile} | gawk '{print $1}') -le 13 ]];then
	    ...
	    python $SCRIPT/Extract_LRR_motifs.py -s $PROTEOME -t ${outfile} -o ${outfile%.tbl}.csv
    fi
    
done

## b. Concatener les motifs trouves
python $SCRIPT/Concat_all_motifs.py -s $PROTEOME -d $(pwd) -o $(pwd)/LRR_${NAME}_concat.tmp
...
do
    ...
    if [[ $(wc -l ${outfile} | gawk '{print $1}') -le 13 ]];then
	    ...
	    python $SCRIPT/Extract_interLRR_motifs.py -s LRR_${NAME}_interLRR.fasta -t ${outfile} -o $(pwd)/${outfile%.tbl}.csv
    fi
    
done

## e. Concatener tous les motifs trouves
python $SCRIPT/Concat_all_motifs.py -s $PROTEOME -d $(pwd) -o $(pwd)/LRR_${NAME}_ALLMOTIFS_init.csv 
...
#processed
python $SCRIPT/Extract_blast_motifs.py -s $PROTEOME -b Resblast.txt -o motifsblast.csv
...
#concat
python $SCRIPT/Concat_all_motifs.py -s $PROTEOME -d $(pwd) -o $(pwd)/LRR_${NAME}_ALLMOTIFS.csv
